...
- **加密 Excel 功能** - 支援 AES-256 加密匯出，提供密鑰保護的檔案安全傳輸
- **Email 發送功能** - 支援將加密的 Excel 檔案直接發送到指定 Email 地址
...
9. **加密 Excel 匯出**：點擊「加密傳輸」→「上傳加密 Excel 到雲端」，輸入密鑰後將檔案加密並下載到本地
10. **Email 發送加密檔案**：在加密匯出對話視窗中輸入收件人 Email，點擊「發送到 Email」將加密檔案直接發送
11. **日期篩選**：使用日期選擇器篩選特定日期之後的待辦事項
12. **標籤篩選**：使用標籤下拉選單篩選特定標籤的待辦事項
13. **重置資料**：點擊「重置資料」按鈕清空本地資料並恢復 Demo 模式
14. **圖卡模式切換**：使用圖卡/列表按鈕切換不同的顯示模式，設定會自動保存
15. **列表模式詳情查看**：在列表模式下點擊標題可查看完整的待辦事項詳情
...
- **加密保護**：支援 AES-256 加密匯出，檔案副檔名為 .aes，需要正確密鑰才能解密開啟
- **Email 發送**：支援將加密的 Excel 檔案直接發送到指定 Email 地址，提供便捷的檔案分享方式
- **密鑰驗證優化**：匯出時只需輸入非空密鑰，匯入時不限制密鑰長度，提供更靈活的密鑰設定
...
- **新增 Email 發送功能**：支援將加密的 Excel 檔案直接發送到指定 Email 地址
- **加密匯出對話視窗優化**：新增 Email 輸入欄位和「發送到 Email」按鈕，提供下載到本地和 Email 發送兩種選擇
- **後端 API 整合**：新增 `/todos/encrypt-todos` API 端點，支援接收加密檔案並發送 Email
- **Email 附件功能**：使用 Nodemailer 將加密檔案作為附件發送到指定收件人
- **密鑰驗證改進**：
  - 匯出時移除 6 位數最小長度限制，只需輸入非空密鑰
  - 匯入時完全移除密鑰長度驗證，提供更靈活的密鑰設定
  - 更新密鑰輸入提示文字為「請輸入密鑰」
- **UI 樣式優化**：「發送到 Email」按鈕樣式與「下載到本地」按鈕保持一致，同時保留禁用狀態的特定樣式
- **使用者體驗提升**：提供更直觀的加密檔案分享方式，支援本地下載和 Email 發送兩種選擇

### 2.11.0

- **新增加密 Excel 功能**：支援 AES-256 加密匯出，提供密鑰保護的檔案安全傳輸
- **加密傳輸對話視窗**：新增「加密傳輸」按鈕，提供加密匯出和匯入功能
- **密鑰保護機制**：加密檔案需要正確密鑰才能解密開啟，確保檔案安全性
- **檔案格式**：加密檔案副檔名為 .aes，使用軍事級別的 AES-256 加密標準
- **下載連結對話框**：加密匯出後顯示下載連結對話框，提供複製連結和直接下載功能
- **安全性提升**：為敏感資料提供額外的安全保護層，適合需要保密傳輸的場景
