...
10. **日期篩選**：使用日期選擇器篩選特定日期之後的待辦事項
11. **標籤篩選**：使用標籤下拉選單篩選特定標籤的待辦事項
12. **重置資料**：點擊「重置資料」按鈕清空本地資料並恢復 Demo 模式
13. **圖卡模式切換**：使用圖卡/列表按鈕切換不同的顯示模式，設定會自動保存
14. **列表模式詳情查看**：在列表模式下點擊標題可查看完整的待辦事項詳情
